...
if ( ! class_exists( 'Formit_Installer' ) ) {
    class Formit_Installer{
        ...
        */
        function run(){
            $this->formit_add_version();
            $this->formit_create_table();
            $this->formit_user_db_create_table();
        ...
        * @return void
        */
        public function formit_add_version(){
        ...
        * @return void
        */
        public function formit_create_table(){
            global $wpdb;
            $formit_forms_name = $wpdb->prefix . 'formit_forms';
            if (!$this->formit_is_table_exists($formit_forms_name)) {
        ...
        * @formit_user_db_create_table
        * @return void
        */
        public function formit_user_db_create_table(){
            global $wpdb;
            $formit_submissions_table = $wpdb->prefix . 'formit_submissions';
            // Check if the table exists before creating it
            if (!$this->formit_is_table_exists($formit_submissions_table)) {
                $charset_collate = $wpdb->get_charset_collate();
        ...
        */
        public function formit_config_table(){
            ...
            if (!$this->formit_is_table_exists($formit_submissions_table)) {
                ...
                $formit_form_settings_data = '{"form_option_checkbox":"on","form_option_date":"on","form_option_files":"on","form_option_header":"on","form_option_hidden":"on","form_option_number":"on","form_option_paragraph":"on","form_option_radio":"on","form_option_select":"on","form_option_text":"on","form_option_textarea":"on","form_attr_description":"on","form_attr_name":"on","form_attr_maxlength":"on","form_attr_style":"on","form_attr_class":"on","form_attr_label":"on","form_attr_placeholder":"on","form_attr_value":"on","form_attr_type":"on","form_attr_required":"on"}';

                if (!$existing_data) {
                    // Insert data if no existing data
                    $data = array(
                        'form_settings' => wp_json_encode($formit_form_settings_data),
        ...
        /**
        * formit_is_table_exists function just checking exists table.
        *
        * @param [string] $table_name
        * @return boolean
        */
        function formit_is_table_exists($table_name) {
            global $wpdb;
